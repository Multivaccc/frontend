...
const Story: FC = () => {
  ...
  const Chat = () => {
    ...
    useEffect(() => {
      const firstMessage : ChatMessage = { type: 'system', content: ['Welcome to '+title] }
      addRandomResponse(firstMessage)
    }, []);

    const addRandomResponse = (userMessage ?: ChatMessage) => {

      const randomResponse = chatData[Math.floor(Math.random() * chatData.length)];
